...
class FFAnalyzerRTSlide:
    ...
    def set_window_size(self, window_size):
        ...
        self.fext_y_smt = np.zeros(self.buffer_len)
    ...
    def update_sensor_data(self, new_smt_fext_value, new_ff_value):
        if self.data_size < self.buffer_len:
            self.fext_y_smt[self.data_size] = new_smt_fext_value
            self.ff_y_buffer[self.data_size] = new_ff_value
        else:
            self.fifo_append(self.fext_y_smt, new_smt_fext_value)
    ...
    def init(self):
        ...
        if self.crt_type == "dep":
            ...
            self.dfext_dff = np.gradient(self.fext_y_smt, self.ff_y_buffer)
            self.dfext_dff_smt = smooth_alternative(self.dfext_dff, self.window_len, window='bartlett', compensate_offset=True)
    ...
    def real_time_detection(self, threshold):
        ...
        if self.crt_type == "dep":
            ...
            new_smt_dfext_dff = incremental_convolve(self.dfext_dff, self.window_len, window='bartlett')
